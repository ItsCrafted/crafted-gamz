...
function exportData(type) {
  const key = type === 'bookmarks' ? 'cg_bookmarks' : 'cg_pins'
  let data = []
  try { data = JSON.parse(localStorage.getItem(key)) || [] } catch {}
  const blob = new Blob([JSON.stringify(data, null, 2)], { type: 'application/json' })
  const a = document.createElement('a')
  a.href = URL.createObjectURL(blob)
  a.download = `cg_${type}_${Date.now()}.json`
  a.click()
...
function renderBookmarks() {
  ...
  if (!list) return
  let bookmarks = []
  try { bookmarks = JSON.parse(localStorage.getItem('cg_bookmarks')) || [] } catch {}
  ...
  bookmarks.slice(0, 8).forEach(bm => {
    ...
    removeBtn.addEventListener('click', () => {
      try {
        let bms = JSON.parse(localStorage.getItem('cg_bookmarks')) || []
        bms = bms.filter(b => b.url !== bm.url)
        localStorage.setItem('cg_bookmarks', JSON.stringify(bms))
      } catch {}
      renderBookmarks()
      updateStats()
...
function updateStats() {
  let bms = []
  try { bms = JSON.parse(localStorage.getItem('cg_bookmarks')) || [] } catch {}
  let pins = []
  try { pins = JSON.parse(localStorage.getItem('cg_pins')) || [] } catch {}
